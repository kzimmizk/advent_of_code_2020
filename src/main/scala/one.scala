...
object one {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val firstAnswer: Result =
      get_pairs().filter(_.total == goal).head

    val secondAnswer: Result =
      get_pairs().filter((x) => data_set.contains(goal - x.total)).map( (x) => Result(x.vals + (goal - x.total))).head

    println(s"First Answer: product of ${firstAnswer.vals} is ${firstAnswer.answer}")
    println(s"Second Answer: product of ${secondAnswer.vals} is ${secondAnswer.answer}")
